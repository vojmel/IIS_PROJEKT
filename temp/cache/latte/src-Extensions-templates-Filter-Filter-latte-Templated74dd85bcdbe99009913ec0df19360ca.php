<?php
// source: C:\wamp64\www\IIS_PROJEKT\vendor\mesour\datagrid\src\Extensions/templates/Filter/Filter.latte

class Templated74dd85bcdbe99009913ec0df19360ca extends Latte\Template {
function render() {
foreach ($this->params as $__k => $__v) $$__k = $__v; unset($__k, $__v);
// prolog Latte\Macros\CoreMacros
list($_b, $_g, $_l) = $template->initialize('916eb2f76a', 'html')
;
// prolog Latte\Macros\BlockMacros
//
// block _filter
//
if (!function_exists($_b->blocks['_filter'][] = '_lbc65b99684d__filter')) { function _lbc65b99684d__filter($_b, $_args) { foreach ($_args as $__k => $__v) $$__k = $__v; $_control->redrawControl('filter', FALSE)
...
<?php $iterations = 0; foreach ($control->parent->getColumns() as $column) { if ($column->hasFiltering() === FALSE) continue ;if ($column instanceof \Mesour\DataGrid\Column\Filter) { if ($column instanceof \Mesour\DataGrid\Column\Date && $control->parent->getDataSource() instanceof \Mesour\DataGrid\ArrayDataSource) continue ;$_b->templates['916eb2f76a']->renderChildTemplate($column->getTemplateFile(), array('column' => $column) + $template->getParameters()) ;} $iterations++; } ?>
